...
public class RePsdTask extends AsyncTask<String,Integer,Integer> {
    private final static String TAG="RePsdTask";//测试标识
    //注册
    private String reg="改密";
    ...
    protected void onPreExecute(){
        //触发监听器的开始事件
        mListener.onRePsdBegin(reg);
    }
    //线程在通报处理进展
    protected void onProgressUpdate(Integer... values){
        //触发监听器的进度更新事件
        mListener.onRePsdUpdate(reg,values[0],0);
    }

    //线程已经完成处理
    protected void onPostExecute(Integer reg_fag){
        //HTTP调用完毕,触发监听器的得到信息事件
        mListener.onFindRePsdInfo(reg_fag,reg);
    }
    //线程已经取消
    protected void onCancelled(String result){
        //触发监听器的取消事件
        mListener.onRePsdCancel(result);
    }

    //声明一个请求信息的监听器对象
    private RePsdTask.OnRePsdListener mListener;

    //设置查询详细地址的监听器
    public void setOnRePsdListener(RePsdTask.OnRePsdListener listener){
        mListener=listener;
    }

    //定义一个请求信息的监听器接口
    public interface OnRePsdListener{//此接口在主activity中实现
        //在线程结束时触发
        void onFindRePsdInfo(Integer reg_fag,String result);
        //在线程处理取消时触发
        void onRePsdCancel(String result);
        //在线程处理过程中更新进度时触发
        void onRePsdUpdate(String request,int progress,int sub_progress);
        //在线程处理开始时触发
        void onRePsdBegin(String request);
    }
}
